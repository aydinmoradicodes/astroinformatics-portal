...
st.markdown("""
    ...
    .stTextInput div[data-baseweb="input"] > div { background-color: #15192b; color: white; }
    </style>
...
if "last_star" not in st.session_state or st.session_state.last_star != target_star:
    ...
    with st.spinner("Connecting to NASA MAST Nodes..."):
        try:
            # STRICT MODE: Only search for 'Kepler' author products.
            # This guarantees the 'quarter' column exists and prevents TESS/K2 crashes.
            search = lk.search_lightcurve(str(target_star).strip(), author="Kepler")
            
            if len(search) > 0:
                # Safely extract quarters (converting to int and sorting)
                ...
                st.session_state.search_error = "Target found in catalog, but no Flux Data available."
...
def fetch_data(star, quarter):
    ...
    try:
        # Strict Search to match Sidebar logic
        ...
        # Pipeline: Remove NaNs -> Sigma Clip -> Flatten
...
if clean_lc is None:
    st.info("Waiting for valid telemetry stream... (Select a target in the sidebar)")
else:
    # --- PHYSICS ---
